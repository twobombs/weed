...
/** SIMD implementation of the double precision complex vector type of 2 complex
 * numbers, only for AVX Apply2x2. */
union complex2 {
  __m256d c2;
  double f[4];
  ...
  inline complex2 operator-(const complex2 &other) const {
    return _mm256_sub_pd(c2, other.c2);
  }
  inline complex2 operator-=(const complex2 &other) {
  ...
  inline complex2 operator*(const double &rhs) const {
    return _mm256_mul_pd(c2, _mm256_set1_pd(rhs));
  }
  inline complex2 operator-() const {
    return _mm256_mul_pd(_mm256_set1_pd(-1.0), c2);
  }
...
inline complex2 mtrxColShuff(const complex2 &mtrxCol) {
  return _mm256_shuffle_pd(mtrxCol.c2, mtrxCol.c2, 5);
}
inline complex2 matrixMul(const complex2 &mtrxCol1, const complex2 &mtrxCol2,
  ...
  return _mm256_add_pd(
      _mm256_fmadd_pd(
          ...
          _mm256_mul_pd(mtrxCol1.c2, _mm256_shuffle_pd(dupeLo, dupeLo, 0))),
      _mm256_fmadd_pd(
          mtrxCol2Shuff.c2,
          _mm256_shuffle_pd(_mm256_xor_pd(SIGNMASK, dupeHi), dupeHi, 15),
          _mm256_mul_pd(mtrxCol2.c2, _mm256_shuffle_pd(dupeHi, dupeHi, 0))));
#else
  return _mm256_add_pd(
      _mm256_add_pd(
          _mm256_mul_pd(
...
inline complex2 operator*(const double &lhs, const complex2 &rhs) {
  return _mm256_mul_pd(_mm256_set1_pd(lhs), rhs.c2);
}
