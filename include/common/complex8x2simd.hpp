...
/** SIMD implementation of the float precision complex vector type of 2 complex
 * numbers, only for COMPLEX_X_2 Apply2x2.
 */
...
union complex2 {
  ...
  inline complex2(const std::complex<float> &cm1,
                  const std::complex<float> &cm2) {
    c2 = _mm_set_ps(cm2.imag(), cm2.real(), cm1.imag(), cm1.real());
  }
  inline complex2(const float &r1, const float &i1, const float &r2,
                  const float &i2) {
    c2 = _mm_set_ps(i2, r2, i1, r1);
  }
  inline std::complex<float> c(const size_t &i) const {
    return complex(f[i << 1U], f[(i << 1U) + 1U]);
  }
  inline complex2 operator+(const complex2 &other) const {
    return _mm_add_ps(c2, other.c2);
  }
...
#if ENABLE_FMA
    // FMA proposed by Elara (OpenAI custom GPT)
    c2 = _mm_fmadd_ps(_mm_shuffle_ps(c2, c2, 177),
                      _mm_xor_ps(SIGNMASK, _mm_shuffle_ps(oVal2, oVal2, 245)),
                      _mm_mul_ps(c2, _mm_shuffle_ps(oVal2, oVal2, 160)));
#else
    c2 = _mm_add_ps(
  ...
  inline complex2 operator*(const float &rhs) const {
    return _mm_mul_ps(c2, _mm_set1_ps(rhs));
  }
  inline complex2 operator-() const {
    return _mm_mul_ps(_mm_set1_ps(-1.0f), c2);
  }
...
inline complex2 mtrxColShuff(const complex2 &mtrxCol) {
  return _mm_shuffle_ps(mtrxCol.c2, mtrxCol.c2, 177);
}
inline complex2 matrixMul(const complex2 &mtrxCol1, const complex2 &mtrxCol2,
  ...
  return _mm_add_ps(
      ...
      _mm_fmadd_ps(
          ...
          _mm_mul_ps(mtrxCol2.c2, _mm_shuffle_ps(dupeHi, dupeHi, 160))));
#else
  return _mm_add_ps(
      _mm_add_ps(
          _mm_mul_ps(mtrxCol1Shuff.c2,
                     _mm_xor_ps(SIGNMASK, _mm_shuffle_ps(dupeLo, dupeLo, 245))),
          _mm_mul_ps(mtrxCol1.c2, _mm_shuffle_ps(dupeLo, dupeLo, 160))),
      _mm_add_ps(
          _mm_mul_ps(mtrxCol2Shuff.c2,
                     _mm_xor_ps(SIGNMASK, _mm_shuffle_ps(dupeHi, dupeHi, 245))),
          _mm_mul_ps(mtrxCol2.c2, _mm_shuffle_ps(dupeHi, dupeHi, 160))));
#endif
}
inline complex2 matrixMul(const float &nrm, const complex2 &mtrxCol1,
                          const complex2 &mtrxCol2,
                          const complex2 &mtrxCol1Shuff,
                          const complex2 &mtrxCol2Shuff,
                          const complex2 &qubit) {
  return matrixMul(mtrxCol1, mtrxCol2, mtrxCol1Shuff, mtrxCol2Shuff, qubit) *
         nrm;
}
inline complex2 operator*(const float &lhs, const complex2 &rhs) {
  return _mm_mul_ps(_mm_set1_ps(lhs), rhs.c2);
}
