...
public:
  ...
  // instances under QUnit. However, we prefer QHybrid! That said, creating
  // these streams once per simulator becomes very expensive in the simulator is
  // trashed and recreated repeatedly.
  cudaStream_t queue;
...
public:
  CUDADeviceContext(int64_t dev_id, int64_t maxAlloc = -1)
      : device_id(dev_id), queue(0), params_queue(0),
        globalLimit((maxAlloc >= 0) ? maxAlloc : -1), preferredSizeMultiple(0U),
    ...
    if (error != cudaSuccess) {
      throw std::runtime_error("CUDA error code on main stream creation: " +
                               std::to_string(error));
    }
    ...
    if (error != cudaSuccess) {
      throw std::runtime_error(
          "CUDA error code on parameter stream creation: " +
          std::to_string(error));
    }
  }
  ...
  size_t GetPreferredConcurrency() {
    ...
    int hybridOffset = 3U;
#if ENABLE_ENV_VARS
    if (getenv("WEED_GPU_OFFSET_QB")) {
      hybridOffset = std::stoi(std::string(getenv("WEED_GPU_OFFSET_QB")));
    }
#endif
...
/** "Weed::CUDAEngine" manages the single CUDA context. */
class CUDAEngine {
public:
  ...
  /// Initialize the CUDA environment. This returns a Weed::CUDAInitResult
  /// object which should be passed to SetDeviceContextPtrVector().
  static InitCUDAResult InitCUDA(std::vector<int64_t> maxAllocVec = {-1});

  /// Get a pointer one of the available CUDA contexts, by its index in the list
  /// of all contexts.
  ...
  /// Pick a default device, for QEngineCUDA instances that don't specify a
  /// preferred device.
  ...
  size_t AddToActiveAllocSize(const int64_t &dev, size_t size) {
    if (dev > ((int64_t)activeAllocSizes.size())) {
      throw std::invalid_argument(
          "OCLEngine::AddToActiveAllocSize device ID is too high!");
    }
