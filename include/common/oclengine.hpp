...
struct OCLKernelHandle {
  ...
  OCLKernelHandle(OCLAPI o, std::string kn) : oclapi(o), kernelname(kn) {}
};
...
public:
  // A cl::Kernel is unique object which should always be taken by reference, or
  // the OCLDeviceContext will lose ownership.
  cl::Kernel &call;
  OCLDeviceCall(const OCLDeviceCall &);

protected:
  OCLDeviceCall(std::mutex &m, cl::Kernel &c) : guard(m), call(c) {}
...
public:
  OCLDeviceContext(cl::Platform &p, cl::Device &d, cl::Context &c,
                   int64_t dev_id, int64_t cntxt_id, int64_t maxAlloc,
    ...
    if (getenv("DISABLE_OOO_OCL")) {
      ...
      queue = cl::CommandQueue(c, d, CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE,
                               &error);
      if (error != CL_SUCCESS) {
        queue = cl::CommandQueue(c, d, 0, &error);
        if (error != CL_SUCCESS) {
...
#else
    queue =
        cl::CommandQueue(c, d, CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, &error);
    if (error != CL_SUCCESS) {
      queue = cl::CommandQueue(c, d, 0, &error);
      if (error != CL_SUCCESS) {
        throw std::runtime_error("Failed to create OpenCL command queue!");
  ...
  OCLDeviceCall Reserve(OCLAPI call) {
    return OCLDeviceCall(*(mutexes[call]), calls[call]);
  }
...
struct InitOClResult {
  ...
  InitOClResult() : all_dev_contexts(), default_dev_context{nullptr} {
...
public:
  ...
  /// Get a pointer to the Instance of the singleton. (The instance will be
  /// instantiated, if it does not exist yet.)
  ...
  static std::string GetDefaultBinaryPath() {
    ...
    return std::string(getenv("HOMEDRIVE") ? getenv("HOMEDRIVE") : "") +
           std::string(getenv("HOMEPATH") ? getenv("HOMEPATH") : "") +
           "\\.weed\\";
#else
    return std::string(getenv("HOME") ? getenv("HOME") : "") + "/.weed/";
#endif
  }
  /// Initialize the OCL environment, with the option to save the generated
  /// binaries. Binaries will be saved/loaded from the folder path "home". This
  /// returns a Weed::OCLInitResult object which should be passed to
  /// SetDeviceContextPtrVector().
  static InitOClResult InitOCL(bool buildFromSource = false,
   ...
   * the result of GetDeviceContextPtrVector(), trims items from it, and sets it
   * with this method, (at initialization, before any QEngine objects depend on
   * them,) all resources associated with the removed items are freed.
   */
  void SetDeviceContextPtrVector(std::vector<DeviceContextPtr> vec,
  ...
  /// Pick a default device, for QEngineOCL instances that don't specify a
  /// preferred device.
  ...
  size_t GetActiveAllocSize(const int64_t &dev) {
    ...
    return (dev < 0) ? activeAllocSizes[GetDefaultDeviceID()]
                     : activeAllocSizes[(size_t)dev];
  }
...
private:
  ...
  static cl::Program MakeProgram(bool buildFromSource, std::string path,
                                 std::shared_ptr<OCLDeviceContext> devCntxt);
  /// Save the program binary:
  static void SaveBinary(cl::Program program, std::string path,
                         std::string fileName);
};
