...
std::vector<DeviceContextPtr> CUDAEngine::GetDeviceContextPtrVector() {
  return all_device_contexts;
}
void CUDAEngine::SetDeviceContextPtrVector(std::vector<DeviceContextPtr> vec,
...
void CUDAEngine::SetDefaultDeviceContext(DeviceContextPtr dcp) {
  default_device_context = dcp;
}
...
InitCUDAResult CUDAEngine::InitCUDA(std::vector<int64_t> maxAllocVec) {
  ...
  // Prefer the last device, for intuitiveness compared to OpenCL Weed.
  ...
  for (int i = 0; i < deviceCount; ++i) {
    ...
    all_dev_contexts.push_back(std::make_shared<CUDADeviceContext>(
        i, maxAllocVec[i % maxAllocVec.size()]));
  }
...
CUDAEngine::CUDAEngine() : maxActiveAllocSizes(1U, -1) {
