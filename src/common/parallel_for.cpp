...
void ParallelFor::par_for_set(const std::set<vecCapIntGpu> &sparseSet,
                              ParallelFunc fn) {
  par_for_inc(
      0U, sparseSet.size(),
...
void ParallelFor::par_for_set(const std::vector<vecCapIntGpu> &sparseSet,
                              ParallelFunc fn) {
  par_for_inc(
      0U, sparseSet.size(),
      [&sparseSet](const vecCapIntGpu &i) {
...
void ParallelFor::par_for_sparse_compose(
    const std::vector<vecCapIntGpu> &lowSet,
    const std::vector<vecCapIntGpu> &highSet, const vecLenInt &highStart,
    ParallelFunc fn) {
...
void ParallelFor::par_for_skip(const vecCapIntGpu begin, const vecCapIntGpu end,
                               const vecCapIntGpu skipMask,
  ...
  if (!lowMask) {
    ...
    incFn = [lowMask, highMask, maskWidth](const vecCapIntGpu &i) {
...
void ParallelFor::par_for_mask(const vecCapIntGpu begin, const vecCapIntGpu end,
