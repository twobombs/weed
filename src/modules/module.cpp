...
ModulePtr Module::load(std::istream &is) {
  ...
  case QRACK_NEURON_LAYER_T: {
    bool b;
    Qrack::QCircuitPtr pre = nullptr;
    Serializer::read_bool(is, b);
    if (b) {
      is >> pre;
    }
    Qrack::QCircuitPtr post = nullptr;
    Serializer::read_bool(is, b);
    if (b) {
      is >> post;
    }
  ...
  default:
    throw std::domain_error("Can't recognize ModuleType " +
                            std::to_string(mtype) + " in Module::load!");
  }
