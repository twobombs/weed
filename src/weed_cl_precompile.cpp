...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    std::cout << "Will save to: " << Weed::OCLEngine::GetDefaultBinaryPath()
              << std::endl;
    Weed::OCLEngine::InitOCL(true, true,
                             Weed::OCLEngine::GetDefaultBinaryPath());
  } else {
    std::cout << "Will save to: " << std::string(argv[1]) << std::endl;
    Weed::OCLEngine::InitOCL(true, true, std::string(argv[1]) + "/");
  }
