...
void benchmarkLoopVariable(std::function<void(Tensor, vecLenInt)> fn,
                           vecLenInt mxQbts, bool logNormal = false) {
  std::cout << std::endl;
  ...
  for (vecLenInt numBits = mnQbts; numBits <= mxQbts; numBits++) {
    // QInterfacePtr qftReg = CreateQuantumInterface(engineStack, numBits,
    // ZERO_BCI, rng, CMPLX_DEFAULT_ARG,
    //     enable_normalization, true, use_host_dma, device_id,
    ...
    std::vector<vecCapInt> qPowers;
    for (vecLenInt i = 0U; i < numBits; ++i) {
    ...
    std::cout << formatTime(stdet, logNormal)
              << ","; /* Sample Std. Deviation (ms) */
    ...
    if (trialClocks.size() < 8) {
      ...
      std::cout << formatTime((trialClocks[trialClocks.size() / 4 - 1] +
                               trialClocks[trialClocks.size() / 4]) /
                                  2,
                              logNormal)
                << ",";
    } else {
      std::cout << formatTime(trialClocks[trialClocks.size() / 4 - 1] / 2,
                              logNormal)
                << ",";
    }
    ...
    if (trialClocks.size() < 4) {
      ...
      std::cout << formatTime((trialClocks[trialClocks.size() / 2 - 1] +
                               trialClocks[trialClocks.size() / 2]) /
                                  2,
                              logNormal)
                << ",";
    } else {
      std::cout << formatTime(trialClocks[trialClocks.size() / 2 - 1] / 2,
                              logNormal)
                << ","; /* Median (ms) */
    }
    ...
    // 3rd Quartile (ms)
    if (trialClocks.size() < 8) {
      std::cout << formatTime(trialClocks[(3 * trialClocks.size()) / 4],
                              logNormal)
                << ",";
    } else if (trialClocks.size() % 4 == 0) {
      ...
      std::cout << formatTime(trialClocks[(3 * trialClocks.size()) / 4 - 1] / 2,
                              logNormal)
                << ",";
    }
...
void benchmarkLoop(std::function<void(Tensor, vecLenInt)> fn,
                   bool logNormal = false) {
  benchmarkLoopVariable(fn, max_qubits, logNormal);
}

TEST_CASE("test_default", "[suite]") {
  benchmarkLoop([](Tensor tensor, vecLenInt n) {});
}
